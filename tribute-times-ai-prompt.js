...
- Return exactly 3 stories, in this exact order, each with its own rule:
  1. THE LEAD STORY (index 0 — this becomes the keepsake's main front-page headline, presented as what happened on the day ${recipientName} ${dateIntro || 'was born'}): its "year" field MUST be exactly ${year} — no other year is acceptable here under any circumstances. It must be a REAL, verified historical event that genuinely happened on ${day}th ${monthName(month)} ${year}. If you cannot find a major globally-famous event on that exact date, use the most notable REAL event you can verify happened somewhere in the world on that exact date in ${year} — a smaller but real and verifiable story is correct; a famous story from the wrong year is NOT acceptable and is a critical error. Do NOT guess or approximate the exact day if unsure — if you are not fully confident an event happened on this precise day AND year, choose a different, well-documented event you ARE confident about, rather than fabricating a plausible-sounding date for a real event.
- IMPORTANT — do not describe this as "the day you were born" in the story text itself unless ${dateMeaning || 'date of birth'} is literally "date of birth" for this occasion. This date field's real meaning is: ${dateMeaning || 'date of birth'}. Phrase any reference to it accordingly (e.g. "the day you graduated," "the day you retired") — never default to birth-announcement language for an occasion that isn't about a birth.${(occasion === "Mother's Day" || occasion === "Father's Day") ? ` EXCEPTION: even though this occasion's date field genuinely is ${recipientName}'s date of birth, do NOT use "the day you were born" / "the day you came into the world" framing anywhere — this occasion celebrates them as a parent, not their own birth. Reference the date/year neutrally instead (e.g. "In ${year}...").` : ''}
  2 & 3. TWO "ON THIS DAY IN HISTORY" STORIES (index 1 and 2 — these are explicitly a different, secondary trivia feature, clearly not claimed to be from the birth year): real verified historical events from ${day}th ${monthName(month)} in any OTHER years, vary the years dramatically (e.g. 1945, 1969, 1815) — this is intentional and desired for these two only, NOT for the lead story above. If a genuinely notable BC/BCE event fits, that's fine to use, but the "year" field MUST be a NEGATIVE number for any BC/BCE date (e.g. -44 for 44 BC) — never a bare positive number for a BC year, since that would be indistinguishable from an AD year of the same number.
...
- LEAD story: body (no more than 280 characters total, ending on a complete sentence). Do NOT exceed 280 characters. (Raised from 220, 11 Aug 2026 — client asked specifically to fill visible blank space in the "News of the Day" lead box with roughly 10 more words; the render-side safety cap for this exact field was raised to match, see tribute-times-renderer.js.)
...
- These limits are hard maximums, not suggestions to undershoot — aim to use at least 80-90% of each character limit. Text that comes in far short of the limit leaves visible empty space in the printed layout — write substantively, not minimally.
- CRITICAL — how to reach that length safely: write in SHORT, SEPARATE complete sentences (each under ~90 characters) rather than one long sentence joined with commas and clauses. Count characters as you go, and stop after the last complete sentence that fits — never leave a final sentence half-finished when you run out of room. A single 200+ character sentence with no period in it is a hard failure, even if the topic is interesting — two or three short, complete sentences are always correct where one long unbroken one is not.
- Headlines must vary in size — lead is biggest, third is smallest

${country.toUpperCase()} NEWS — On This Day:
- Return 3-4 items. The FIRST item's "year" field MUST be exactly ${year} (same hard rule as the world news lead story above — this is displayed directly under the main headline as more of "this exact date" reporting, not trivia). If nothing globally notable happened in ${country} on that exact date, use a smaller but REAL, verifiable local event from that exact date instead — never substitute a different year. Same tone rule as above: do not default to "the day you were born" phrasing unless this occasion is actually about a birth. This FIRST item's body specifically: no more than 280 characters, ending on a complete sentence — do NOT exceed 280 characters. (Raised from 220, 11 Aug 2026, same "News of the Day" blank-space request as the world news lead above — applies only to this first item, not the trivia items below.)
- The remaining items (index 1 onward) are "on this day in history" trivia from different years, all on ${day}th ${monthName(month)} — this varied-year trivia format is intentional for these only. Each of these items' body: no more than 160 characters, ending on a complete sentence (the first item's own body length is specified separately above). Prefer 1–2 short complete sentences — never one long comma-joined sentence that has to be cut mid-phrase on the printed page.
- Must be genuinely relevant to ${country} — local events, not international
- Every item, including the first: year, short headline, body, AND a "country" field naming the country the event actually took place in. This will almost always be "${country}" — but if you include a regional/Commonwealth/international event because nothing purely domestic fits (see the rule below), set "country" to where that event genuinely happened, not "${country}". This keepsake is displayed with a per-event country label, so a Bureau-of-Investigation-in-Washington-style event must never be labelled with a country it didn't happen in.
...
${(occasion === "Mother's Day" || occasion === "Father's Day") ? `- IMPORTANT: even though the date entered is ${recipientName}'s actual date of birth, this occasion celebrates them as a parent, not their birth — do NOT write "on the day you were born," "the day you came into the world," or similar birth-announcement phrasing anywhere in this message. Reference the historical facts neutrally (e.g. "In ${year}..." or "That same year...") and keep the emotional focus on celebrating them as a mother/father, not on their own birth.` : ''}
