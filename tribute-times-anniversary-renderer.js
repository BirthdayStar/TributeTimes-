// ============================================================
// THE TRIBUTE TIMES — ANNIVERSARY / WEDDING HTML RENDERER (A4 PRINT-LOCKED)
// Version 1.0 — August 2026
// A genuinely separate rendering path for couple-based occasions
// (Anniversary, Golden/Silver/Diamond Anniversary, Wedding Day) — see
// new_changes.md Step 13. Same print-locked A4 layout geometry as
// tribute-times-renderer.js (occasion-neutral print scaffolding), but:
// both names flow through the banner/headline/message instead of one
// name being collected and ignored; no "born"/"world welcomes" language;
// the counter reads "years married" (computed wedding date -> today,
// which is what an anniversary actually measures) instead of "days old";
// and the celebrity-birthdays trivia heading is reframed so it doesn't
...
function vintageAnniversaryHoroscope(signName) {
  const horoscopes = {
    Aries: "The stars have long favoured bold beginnings, and yours was no exception. Energy and pioneering spirit have carried you through the years together. Continued warmth and adventure lie ahead.",
    Taurus: "A steady, grounded partnership was always written in the stars for you both. Patience and loyalty have been your quiet strengths. More stability and shared joy lie ahead.",
    Gemini: "Lively conversation and shared curiosity have kept your years together vibrant. Communication has been the heart of your bond. New adventures in learning and laughter still await.",
    Cancer: "A deeply nurturing bond, rooted in home and heart. Sensitivity and warmth have carried you through the years. Family and comfort remain your greatest treasures.",
    Leo: "Radiance and generosity of spirit have defined your years together. Your warmth has touched all who know you. Joyful celebration continues to suit you both well.",
    Virgo: "A thoughtful, devoted partnership built on care and attention to one another. Your steady dedication has not gone unnoticed. Continued harmony lies ahead.",
    Libra: "Balance and harmony have been the hallmark of your years together. A shared appreciation for beauty and fairness runs deep. More grace and partnership await.",
    Scorpio: "An intense, loyal bond that has weathered every season. Passion and depth have defined your years together. Continued transformation and closeness lie ahead.",
    Sagittarius: "An adventurous spirit has carried your years together to new horizons. Optimism has been your shared compass. More journeys, together, still await.",
    Capricorn: "Patience and steady dedication have built something lasting. Your shared discipline has been rewarded over the years. Continued security and quiet contentment lie ahead.",
    Aquarius: "An independent, unconventional bond that has always marched to its own rhythm. Shared ideals have kept you close. More surprises and shared purpose await.",
    Pisces: "A gentle, intuitive connection has guided your years together. Compassion has been your quiet strength. Continued dreams, shared together, still lie ahead.",
  };
  return horoscopes[signName] || "The years together have been marked by steady growth, shared warmth, and quiet devotion. The path ahead holds more of the same.";
}

function renderAnniversaryNewspaper(data, content, fonts) {
  const {
    recipientName, partnerName, dateFormatted, dateLong, day, month, year,
    country, occasion, bannerText, senderName,
    edition, currency, yearsMarried, personalMessage, localIndexLabel,
  } = data;

  const cleanedRecipientName = titleCase(recipientName);
  const cleanedPartnerName = partnerName ? titleCase(partnerName) : '';
  ...
  // Client-flagged wording (7 Aug 2026 QA session, bug #11): a same-day
  // Wedding Day keepsake computed 0 years married, correctly, but "0
  // years married" reads oddly on the wedding day itself — replaced with
  // a dedicated same-day line rather than the numeric counter.
  const yearsMarriedStr = !Number.isFinite(yearsMarried)
    ? ''
    : yearsMarried === 0
  ...
  // ── ON THIS DAY, THROUGH HISTORY (reframed heading — same underlying
  // Wikipedia-sourced facts anchored to the wedding date, per client
  // instruction: keep the facts, drop the birth-specific framing) ──
  ...
  const trivialHTML = birthdays.slice(0, 3).map((b, i) => `
  ...
  // re-check, 8 Aug 2026): checking existence alone let a story with no
  // `body` field (AI schema non-compliance) through as a blank box
  // ("World, 1969:" with nothing after it). Now requires a genuine body,
  // falling through to business[] and, as a last resort, either
  // headline — never both bodies AND both headlines missing.
  ...
  const horoscopeText = cleanTruncate(vintageAnniversaryHoroscope(astro.starSign.name), 230);

  // ── NAME BANNER — both names, never a single name standing in for a couple ──
  const bannerFullText = `${bannerText.toUpperCase()} — ${coupleDisplayName.toUpperCase()}`;
  const bannerLen = bannerFullText.length;
  const bannerFontSize = bannerLen <= 33 ? 21 : bannerLen <= 36 ? 18 : bannerLen <= 40 ? 16 : bannerLen <= 45 ? 14 : bannerLen <= 55 ? 12 : 10;

  // ── LEAD HEADLINE / SUB-HEADLINE — wedding framing, never birth framing ──
  const leadHeadline = bannerText;
  const leadSub = cleanTruncate(`Married ${dateFormatted} &mdash; ${country}`, 105);
...
<title>The Tribute Times — ${coupleDisplayName}</title>
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .ticker .titems { display: flex; flex-grow: 1; justify-content: space-between; overflow: hidden; }
  .ticker .tick { display: flex; align-items: center; gap: 1.5mm; }
  .ticker .tn { text-transform: uppercase; color: #b9b09a; }
  .ticker .tv { font-weight: 600; }
  .ticker .tu { color: #5ad05a; }
  .ticker .td { color: #ff7878; }
  ...
  ol.chart li .entry-text { min-width: 0; overflow: hidden; white-space: nowrap; text-overflow: ellipsis; }
  ...
  <div class="cols">
    ...
    <div class="col">
      ...
      <section class="s-birthdays">
        <h3>On This Day, Through History</h3>
        ${trivialHTML}
      </section>
      ${booksHTML ? `
...
module.exports = { renderAnniversaryNewspaper };
